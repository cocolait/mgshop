...
class Rbac
{
    ...
    {
        $sql = "SELECT * FROM " . $this->user_role_table . " u " . " JOIN " . $this->role_table . " r ON ur.role_id = r.id " . " WHERE ur.admin_id=" . $id;
    ...
    {
        $sql = "SELECT n.id,n.name,n.pid,n.level,n.title,n.show FROM " . $this->user_role_table . " ur" . " JOIN " . $this->role_table . " r ON ur.role_id = r.id " . " JOIN " . $this->access_table . " a ON a.role_id=r.id " . " JOIN " . $this->node_table . " n ON n.id = a.node_id" . " WHERE ur.admin_id=" . $id . " AND n.status=1 AND r.status=1" . " GROUP BY a.node_id" . " ORDER BY n.sort ASC";
    ...
    public function verify()
    {
        ...
        foreach ($_SESSION['__RBAC__'] as $module => $a)
        {
            ...
            if ($module == strtolower(MODULE))
            {
                ...
                {
                    if (isset($a['_NODE_'][strtolower(CONTROLLER)]['_NODE_'][ACTION]))
                    {
    ...
    public function isSuperUser()
    {
        ...
        $uname = Db::table(C('rbac.user_table'))->where($this->auth_key, $_SESSION[$this->auth_key])->pluck('uname');
        if ($uname == C('rbac.super_user'))
        {
